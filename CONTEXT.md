...
This project is a local OpenAI-compatible HTTP gateway for Codex app-server. It lets examples written for the official OpenAI Python SDK use `OpenAI(base_url="http://127.0.0.1:<port>/v1", api_key=<local-gateway-token>)` while keeping Codex credentials and privileged app-server capabilities out of the OpenAI-style request surface.
...
- Local gateway token: the bearer token accepted by this local HTTP server. It is not an OpenAI API key and must not be proxied, persisted, or confused with Codex auth.
...
- Unsupported feature policy: the gateway returns explicit OpenAI-style errors for API features it cannot honor instead of silently ignoring or pretending to support them.
