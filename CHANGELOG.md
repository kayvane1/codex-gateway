...
- Expects a Codex CLI/app-server install that supports `codex app-server --listen stdio://` and the app-server methods used by the gateway client.
- Leaves generated protocol artifacts out of git; use `codex-gateway generate-json-schema` or `codex-gateway generate-ts` to recreate them locally under ignored `generated/`.
